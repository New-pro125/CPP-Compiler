...
string SymbolTable::getIRName(const string &symbol_name)
{
    Symbol *sym = this->lookup(symbol_name);
    return sym ? sym->irName : symbol_name;
}
bool SymbolTable::insert(const string &symbol_name, Symbol sym)
{
    ...
    if (!sym.isFunction)
    {
        ...
        else
        {
            ...
            sym.irName = sym.name + "@" + to_string(symbolVersions[symbol_name]);
        }
    }
...
const vector<pair<string, string>> &SymbolTable::getRenameEvents() const
{
    return renameEvents;
}

vector<Symbol> SymbolTable::getUnusedSymbolsInCurrentScope()
{
    vector<Symbol> unused;
...
Symbol *SymbolTable::lookup(const string &symbol_name)
{
...
{
    cout << "\nSymbol Table\n";
    cout << left << setw(20) << "Name" << setw(10) << "Type" << setw(8)

         << "Scope" << setw(8) << "Const" << setw(8) << "Init" << setw(8) << "Used"
         << setw(8) << "Func" << setw(8) << "Line" << endl;
    cout << string(78, '-') << endl;
    for (int i = 0; i < (int)this->archive.size(); i++)
    {
        ...
        if (sym.isRemoved)
            continue;
        cout << left << setw(20) << sym.irName << setw(10) << typeToString(sym.dataType) << setw(8)
             << sym.scopeLevel << setw(8)
             << (sym.isConst ? "yes" : "no")
             << setw(8) << (sym.isInitialized ? "yes" : "no") << setw(8) << (sym.isUsed ? "yes" : "no")
             << setw(8) << (sym.isFunction ? "yes" : "no") << setw(8) << sym.declaredLine << endl;
        if (sym.isFunction)
        {
            cout << "Return: " << typeToString(sym.returnType) << "| Params: ";
            for (size_t p = 0; p < sym.paramTypes.size(); p++)
            {
                if (p > 0)
                    cout << ", ";
                cout << typeToString(sym.paramTypes[p]) << " " << sym.paramNames[p];
                if (p < sym.defaultValues.size() && !sym.defaultValues[p].empty())
                    cout << " = " << sym.defaultValues[p];
            }
            cout << endl;
        }
    }
...
bool SymbolTable::removeCurrentScopeSymbol(const string &symbol_name)
{
